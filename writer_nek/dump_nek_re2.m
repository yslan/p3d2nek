function dump_nek_re2(fname,X,Hexes,CBC,iforder2,Hex20,verbose)
%function dump_nek_re2(fname,X,Hexes,sph_hex,facet_hex,iFtoUF,UFpair,CBC,cbc_nek)
t0=tic;

assert(iforder2==0,'dump_nek_re2: curved size is not supported') %TODO
...
ndim=size(X,2); E=size(Hexes,1); nv=size(Hexes,2); nface=2*ndim;
...
nH=size(Hexes,1); fname=[fname '.re2'];
